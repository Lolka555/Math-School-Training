    ...
    def schet(self):
        ...
        if self.name.isdigit() is True:
            if int(self.name) == self.spis[2] - self.spis[1]:
                self.cur.execute('UPDATE core SET Score2 == ? WHERE Name == ?', (1, self.lastname))               
            else:
                self.cur.execute('UPDATE core SET Score2 == ? WHERE Name == ?', (0, self.lastname))
        else:
            self.cur.execute('UPDATE core SET Score2 == ? WHERE Name == ?', (0, self.lastname))
        self.base.commit()
        self.cur.close()
