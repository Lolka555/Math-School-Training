...
import sqlite3
from End import End3

class Work(QWidget):
    def var(self):
        self.variant = '//'
        self.firstchis = randint(0, 3000)
        self.secondchis = randint(1, self.firstchis)
        self.secondchis = self.firstchis // self.secondchis
    ...
    def schet(self):
        ...
        if self.name.isdigit() is True:
            if int(self.name) == self.spis[2] // self.spis[1]:
                self.cur.execute('UPDATE core SET Score4 == ? WHERE Name == ?', (1, self.lastname))               
            else:
                self.cur.execute('UPDATE core SET Score4 == ? WHERE Name == ?', (0, self.lastname))
        else:
            self.cur.execute('UPDATE core SET Score4 == ? WHERE Name == ?', (0, self.lastname))
        self.base.commit()
        self.cur.close()
